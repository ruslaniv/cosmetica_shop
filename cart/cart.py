...
class Cart(object):
  ...
  def __init__(self, request):
    ...
    self.cart = cart
    self.coupon_id = self.session.get('coupon_id') # инициализация купона в текущей сессии
  ...
  @property
  def coupon(self):     # метод определяется как свойство (property). Если в сессии присутствует атрибут
    if self.coupon_id:  # coupon_id, то создается новый экземпляр класса Купон с данным ID
      try:
  ...
  def get_discount(self): # размер скидки
    if self.coupon:
      return (self.coupon.discount / Decimal(100)) * self.get_total_price()
    return Decimal(0)

  def get_total_price_after_discount(self): # всего с учетом скидки
